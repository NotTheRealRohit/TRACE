...
def plot_derived_features():
    ...
    fig, ax = create_figure(7.5, 4.0)

    ax.text(3.75, 9.2, "Derived Feature Engineering", ha='center', fontsize=11, fontweight='bold')
    ...
    for i in range(len(raw_cols)):
        add_arrow(ax, (2.0, 7.9 - i * 0.9), (3.2, 7.9 - i * 0.9))
    ...
    for i, (label, content) in enumerate(bins):
        ...
        ax.text(6.2, 7.5 - i * 0.9 - 0.15, content, ha='left', va='center', fontsize=6, family='monospace')

    for i in range(len(raw_cols)):
        add_arrow(ax, (6.0, 7.9 - i * 0.9), (7.2, 7.9 - i * 0.9))
...
def plot_six_stage_pipeline():
    ...
    for i in range(len(stages) - 1):
        add_arrow(ax, (stages[i][0] + 1.0, 7.15), (stages[i+1][0] - 1.0, 7.15))
...
def plot_data_loading_pipeline():
    ...
    for x, name, content in steps:
        add_process_box(ax, x - 0.7, 6.5, 1.4, 1.2, name, fontsize=9)
        ax.text(x, 6.0, content, ha='center', va='center', fontsize=6, family='monospace')

    for i in range(len(steps) - 1):
        add_arrow(ax, (steps[i][0] + 0.7, 7.1), (steps[i+1][0] - 0.7, 7.1))
...
def plot_train_test_split():
    ...
    add_arrow(ax, (3.25, 7.5), (3.25, 6.0))
...
def plot_complaint_matching():
    ...
    add_arrow(ax, (2.5, 7.45), (3.1, 7.45))
    add_process_box(ax, 3.1, 7.0, 1.8, 0.9, "Keyword Scan\n(first match wins)")

    add_arrow(ax, (4.9, 7.45), (5.5, 7.45))
    ax.add_patch(FancyBboxPatch((5.5, 6.5), 1.2, 0.9, boxstyle="round,pad=0.1", facecolor=COLORS['success'], edgecolor=COLORS['success'], linewidth=1.2))
    ax.text(6.1, 7.0, "Match?", ha='center', va='center', fontsize=8, fontweight='bold', color='white')

    add_arrow(ax, (6.1, 6.5), (6.1, 6.0))
    ax.text(6.8, 6.25, "Yes", ha='left', fontsize=7, color=COLORS['success'], fontweight='bold')
    ax.text(4.6, 6.25, "No", ha='right', fontsize=7, color=COLORS['danger'], fontweight='bold')

    add_process_box(ax, 3.1, 4.8, 1.8, 0.9, "Fuzzy Match\ncutoff=0.25", facecolor=COLORS['bg_box'], edgecolor=COLORS['secondary'])

    add_arrow(ax, (4.9, 5.25), (5.5, 5.25))
    ax.add_patch(FancyBboxPatch((5.5, 4.5), 1.2, 0.9, boxstyle="round,pad=0.1", facecolor=COLORS['success'], edgecolor=COLORS['success'], linewidth=1.2))
    ax.text(6.1, 5.0, "Match?", ha='center', va='center', fontsize=8, fontweight='bold', color='white')

    add_arrow(ax, (6.1, 4.5), (6.1, 4.0))
    ax.text(6.8, 4.25, "Yes", ha='left', fontsize=7, color=COLORS['success'], fontweight='bold')
    ax.text(4.6, 4.25, "No", ha='right', fontsize=7, color=COLORS['danger'], fontweight='bold')
    
    add_output_box(ax, 3.1, 2.5, 1.8, 0.9, "OBD Light ON\n(default)")
    add_output_box(ax, 6.1, 2.5, 1.8, 0.9, "Complaint Label")
...
def plot_rule_engine_flow():
    ...
    for i in range(len(rules) - 1):
        add_arrow(ax, (2.1, rules[i][0] - 0.4), (2.1, rules[i+1][0] + 0.4))
...
def plot_stage1_llm_understanding():
    ...
    ax.text(4.0, 9.0, "Stage 1: LLM Claim Understanding", ha='center', fontsize=11, fontweight='bold')

    add_input_box(ax, 0.5, 7.0, 2.0, 1.2, "notes + DTC")
    add_arrow(ax, (2.5, 7.6), (3.2, 7.6))
    add_llm_box(ax, 3.2, 7.0, 2.5, 1.2, "LLM: understand_claim()\nunderstand_claim_with_retry()")
    add_arrow(ax, (5.7, 7.6), (6.4, 7.6))
    add_output_box(ax, 6.4, 7.0, 2.5, 1.2, "Output:\n{category, complaint,\nseverity, FA, reasoning,\nconfidence}")
...
def plot_stage3_feature_translation():
    ...
    ax.text(4.0, 9.5, "Stage 3: Feature Translation", ha='center', fontsize=11, fontweight='bold')

    add_input_box(ax, 0.5, 7.0, 1.8, 1.2, "notes + DTC")

    ax.text(1.8, 5.8, "LLM path:", ha='center', fontsize=8, fontweight='bold', color=COLORS['brown'])
    add_arrow(ax, (2.3, 7.6), (3.0, 7.6))
    add_llm_box(ax, 3.0, 7.0, 2.5, 1.2, "translate_to_ml_features()\n(if LLM available)")

    ax.text(1.8, 4.5, "Fallback path:", ha='center', fontsize=8, fontweight='bold', color=COLORS['gray_medium'])
    add_arrow(ax, (2.3, 6.0), (3.0, 6.0))
    add_process_box(ax, 3.0, 5.4, 2.5, 1.2, "extract_dtc_features()\n+ match_complaint()\n(deterministic)")

    add_arrow(ax, (5.5, 7.6), (6.5, 7.6))
    add_arrow(ax, (5.5, 6.0), (6.5, 7.6))

    add_output_box(ax, 6.5, 6.5, 2.8, 2.0, "Feature Dict:\n{customer_complaint,\ndtc_codes, dtc_text,\ndtc_count, has_P/U/C/B,\nvoltage, mileage_km,\nyear, ...}")
...
def plot_stage4_xgboost_cascade():
    ...
    fig, ax = create_figure(8.5, 5.0)

    ax.text(4.25, 9.5, "Stage 4: XGBoost Cascade Scoring", ha='center', fontsize=11, fontweight='bold')

    add_input_box(ax, 0.5, 7.5, 2.2, 1.2, "Feature Vector\n(12 transformers)")
    add_arrow(ax, (2.7, 8.1), (3.4, 8.1))

    add_process_box(ax, 3.4, 7.5, 2.0, 1.2, "FA Classifier\n(clf_fa)")
    add_arrow(ax, (4.4, 7.5), (4.4, 5.8))

    ax.text(5.4, 6.5, "p_FA (6 class probabilities)", ha='center', fontsize=7, color=COLORS['purple'])

    add_box(ax, 3.4, 5.0, 2.0, 0.8, "Concatenate:\n[X | p_FA]", facecolor=COLORS['purple'], edgecolor=COLORS['purple'], fontsize=7, fontcolor='white')
    add_arrow(ax, (4.4, 5.0), (4.4, 4.7))

    add_process_box(ax, 3.4, 3.5, 2.0, 1.2, "WD Classifier\n(clf_wd)")
    add_arrow(ax, (5.4, 4.1), (6.8, 4.1))
    add_output_box(ax, 6.8, 3.5, 2.8, 1.8, "Output:\nWD prediction\np_WD probabilities\nconfidence = √(c_FA·c_WD)")
...
def plot_stage5_score_combination():
    ...
    ax.text(4.0, 9.0, "Stage 5: Score Combination", ha='center', fontsize=11, fontweight='bold')

    add_box(ax, 0.5, 7.0, 1.6, 1.2, "Rule\nResult", facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=8, fontcolor='white')
    add_box(ax, 2.5, 7.0, 1.6, 1.2, "ML\nResult", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=8, fontcolor='white')
    add_box(ax, 4.5, 7.0, 1.6, 1.2, "LLM\nResult", facecolor=COLORS['brown'], edgecolor=COLORS['brown'], fontsize=8, fontcolor='white')

    add_arrow(ax, (6.1, 7.6), (6.7, 7.6))
    add_process_box(ax, 6.7, 7.0, 2.2, 1.2, "combine_scores()\nWeighted blend\n+ Agreement check")

    add_arrow(ax, (8.9, 7.6), (9.5, 7.6))
    add_output_box(ax, 9.5, 7.0, 2.2, 1.2, "Output:\ncombined_confidence\nstatus\ndecision_engine")
...
def plot_stage6_output_formatting():
    ...
    fig, ax = create_figure(7.5, 4.5)

    ax.text(3.75, 9.5, "Stage 6: Output Formatting", ha='center', fontsize=11, fontweight='bold')

    add_input_box(ax, 0.5, 7.0, 2.5, 1.2, "combined_decision\n+ features")

    ax.text(1.75, 5.8, "LLM path:", ha='center', fontsize=8, fontweight='bold', color=COLORS['brown'])
    add_arrow(ax, (3.0, 7.6), (3.6, 7.6))
    add_llm_box(ax, 3.6, 7.0, 2.0, 1.2, "format_output()\nNatural language\nreason generation")

    ax.text(1.75, 4.5, "Fallback:", ha='center', fontsize=8, fontweight='bold', color=COLORS['gray_medium'])
    add_arrow(ax, (3.0, 6.0), (3.6, 6.0))
    add_process_box(ax, 3.6, 5.4, 2.0, 1.2, "assemble_output_from_fields()\nTemplate-based reason")

    add_arrow(ax, (5.6, 7.6), (6.5, 7.8))
    add_arrow(ax, (5.6, 6.0), (6.5, 7.8))
    add_output_box(ax, 6.5, 6.5, 2.5, 2.0, "ClaimResponse:\nstatus, FA, WD,\nconfidence, reason,\nmatched_complaint,\ndecision_engine")
...
def plot_fastapi_architecture():
    ...
    add_arrow(ax, (3.25, 4.8), (3.25, 3.2))
...
def plot_docker_compose_architecture():
    ...
    add_arrow(ax, (2.0, 6.5), (2.75, 5.3))
    add_arrow(ax, (3.25, 5.3), (3.0, 6.5))
    add_arrow(ax, (5.5, 6.5), (3.25, 5.3))
    add_arrow(ax, (3.25, 4.9), (4.5, 6.5))
