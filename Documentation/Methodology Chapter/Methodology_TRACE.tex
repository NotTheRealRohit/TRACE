...
\lstdefinestyle{Python}{
  morekeywords={self,cls,None,True,False},
  emph={self,cls},
  emphstyle=\color{violet},
}
\lstdefinestyle{JavaScript}{
  language=JavaScript,
  morekeywords={async,await,const,let,var,function,return,if,else,for,try,catch},
  keywordstyle=\color{blue},
  stringstyle=\color{red!70!black},
  commentstyle=\color{green!50!black},
}
...
where the condition ``LLM has signal'' is defined as
$\text{confidence}_{\text{LLM}} \geq 0.5$, using the constant
\texttt{LLM\_CONFIDENCE\_FOR\_TAG = 0.5}
(\texttt{ml\_predictor.py:652}).
The tag assignment is implemented at lines 780--787 of \texttt{ml\_predictor.py}.

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_hybrid_decision_engine_overview.pdf}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_six_stage_pipeline.pdf}
    }}
...
\begin{equation}
\begin{split}
\text{FA}\colon \mathcal{X} \to \{&\text{NTF},\; \text{Track burnt due to EOS},\; \text{ASIC CJ327 failure due to EOS},\\
&\text{Sensor short due to moisture},\; \text{Connector damage},\; \text{Controller failure}\}
\end{split}
\label{eq:fa-mapping}
\end{equation}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_dataset_overview.pdf}
    }}
    \caption{Dataset schema overview with example rows illustrating the column structure and data types.}
    \label{fig:dataset-overview}
\end{figure}

The dataset is generated by \texttt{generate\_dataset\_v9(1).py}
(779~lines) and saved as
\texttt{synthetic\_warranty\_}\linebreak
\texttt{claims\_v9.csv}.
Reproducibility is ensured through seeded random number generation:
\texttt{rng = np.random.default\_rng(42)} at line~22, and
\texttt{random.seed(42)} at line~23.
The generator implements 10~named improvements over previous versions
(C4, C5, C6, W5, W6, W7, DTC1--DTC4), each documented in the module
header (lines 1--14).
The dataset is loaded at \texttt{ml\_predictor.py:94}:
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_fa_class_distribution.pdf}
    }}
...
The voltage distributions are generated using
\texttt{truncated\_normal()}
(\texttt{generate\_dataset\_v9(1).py:206--213}).
The implementation uses the inverse CDF method:
it draws a uniform random value and applies
\texttt{truncnorm.ppf()} to obtain the corresponding
quantile from the truncated normal distribution.
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_voltage_distributions.pdf}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_mileage_distributions.pdf}
    }}
...
\label{tab:dtc-pools}
\small
\begin{tabular}{lR{2.5cm}cR{6.5cm}}
\toprule
\textbf{FA Class} & \textbf{Pool Variable} & \textbf{Count} & \textbf{Representative Codes} \\
\midrule
ASIC CJ327 failure & \texttt{DTC\_ASIC} (primary + sec.) & 16 & P0601, P0602, P0562,\\
& & & P0563, P0611, P0613, P0634 \\
Track burnt due to EOS & \texttt{DTC\_TRACK} & 31 & P0562, P0300--P0306,\\
& & & P0480--P0482, P0351--P0356 \\
Sensor short (moisture) & \texttt{DTC\_SENSOR\_MOISTURE} & 20 & P0113, P0112, P0118,\\
& & & P0117, P0128, P0038 \\
Connector damage & \texttt{DTC\_CONNECTOR} + sec. & 13+9 & C0031, C0036, C0045,\\
& & & B1234, B1031, P0340, P0325 \\
...
The cross-FA injection uses \texttt{DTC\_AMBIGUOUS\_CROSS}
(8 codes, lines 39--48: P0300, P0171, P0325, P0340, U0100, U0155,
P0500, P0420) with \texttt{CROSS\_FA\_INJECT\_RATE = 0.04} (line~49).
The function pre-filters the native pool to ensure that injected codes
are genuinely foreign---codes already in the FA class's native pool are
excluded from injection (lines 224--225).

The ASIC primary pool was expanded from 10 to 12 codes in v9
(DTC1, lines 60--63): \texttt{DTC\_ASIC\_PRIMARY} now includes 12 codes
(P0601, P0602, P0604, P0605, P0606, P0607, P0608, P0610, P0562, P0563,
P0611, P0613), with a separate 4-code secondary pool
(\texttt{DTC\_ASIC\_SECONDARY}: P0634, P068A, P068B, P0686).
Primary codes are drawn with 88\% probability of a single-code DTC;
otherwise a second code is drawn from the full ASIC pool
(lines 333--339).
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_dtc_pool_architecture.pdf}
    }}
...
Warranty decisions are assigned probabilistically based on FA class
and contextual factors (voltage thresholds, mileage), with
class-specific probability distributions that reflect real-world
warranty adjudication patterns. The probabilities encode the business
logic that certain failure modes are predominantly supplier
responsibility (Production Failure), others are customer responsibility
(Customer Failure), and some represent normal operation
(According to Specification).

The ASIC warranty decision is voltage-conditional,
implemented at \texttt{generate\_dataset\_v9(1).py:344--349}:
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_wd_probabilities.pdf}
    }}
...
Label noise is injected post-generation to simulate real-world
annotation errors, ambiguous boundary cases, and mileage-zone
uncertainty. Six distinct noise mechanisms target specific FA classes
with class-appropriate flip rates. The noise is applied only to the
Warranty Decision label---the Failure Analysis label is not
modified---because FA represents the objective technical root cause
while WD represents a potentially subjective adjudication decision.

The noise injection is implemented in
\texttt{inject\_warranty\_label\_noise()}
(\texttt{generate\_dataset\_v9(1).py:515--595})
with a base noise rate of $\eta = 0.015$.
The six mechanisms are shown in
Table~\ref{tab:label-noise}.
...
\small
\begin{tabular}{lR{2cm}R{2.2cm}cR{4.5cm}}
\toprule
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_label_noise_mechanisms.pdf}
    }}
...
The null handling code is implemented at \texttt{ml\_predictor.py:281--284}:
...
DTC features are extracted at line~291 via
\texttt{dtc\_feats = pd.DataFrame(}\linebreak
\texttt{list(df["DTC"].apply(extract\_dtc\_features)))}.
This produces a DataFrame with columns for DTC count, prefix flags
(has\_P, has\_U, has\_C, has\_B), DTC text, and one-hot flags for
90+ high-value DTC codes.
See Section~\ref{sec:dtc-feature-extraction} for details.

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_data_loading_pipeline.pdf}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_train_test_split.pdf}
    }}
...
\begin{table}[H]
\centering
\caption{Derived feature specifications: feature type, binning thresholds, and design rationale. Computed after the train/test split.}
\label{tab:derived-features}
\small
\begin{tabular}{llR{2.3cm}R{5.5cm}}
\toprule
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_derived_features.pdf}
    }}
...
\small
\begin{tabular}{llR{2.5cm}R{4cm}}
\toprule
\textbf{Transformer} & \textbf{Type} & \textbf{Output Shape} & \textbf{Fit On / Source} \\
\midrule
OHE (complaint)         & OneHotEncoder     & sparse $(n \times |\mathcal{C}|)$   & \texttt{ohe.fit\_tr(df\_tr)} \\
TF-IDF (DTC text)       & TfidfVectorizer   & sparse $(n \times 40)$              & \texttt{tfidf\_d.fit\_tr(dtc\_tr)} \\
DTC flags               & raw values        & dense $(n \times (5 + |\text{HV}|))$ & \texttt{dtc\_tr[dtc\_flag\_cols]} \\
OHE (supplier)          & OneHotEncoder     & sparse $(n \times 5)$               & \texttt{ohe\_sup.fit\_tr} \\
Scaler (mileage)        & StandardScaler    & dense $(n \times 1)$                 & \texttt{mileage\_sc.fit\_tr} \\
Scaler (year)           & StandardScaler    & dense $(n \times 1)$                 & \texttt{year\_sc.fit\_tr} \\
OHE (mileage bracket)   & OneHotEncoder     & sparse $(n \times 4)$               & \texttt{ohe\_mileage.fit\_tr} \\
Scaler (claim age)      & StandardScaler    & dense $(n \times 1)$                 & \texttt{claim\_age\_sc.fit\_tr} \\
Scaler (voltage)        & StandardScaler    & dense $(n \times 1)$                 & \texttt{voltage\_sc.fit\_tr} \\
OHE (voltage bracket)   & OneHotEncoder     & sparse $(n \times 7)$               & \texttt{ohe\_volt.fit\_tr} \\
OHE (DTC count bracket) & OneHotEncoder     & sparse $(n \times 4)$               & \texttt{ohe\_dtc\_cnt.fit\_tr} \\
Interactions            & raw values        & dense $(n \times 4)$                 & \texttt{df\_tr[volt\_hi\_P, \ldots]} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_transformer_pipeline.pdf}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_dtc_feature_extraction.pdf}
    }}
...
The complete \texttt{extract\_dtc\_features()} function is implemented at \texttt{ml\_predictor.py:229--243}:
...
\text{get\_close\_matches}(\text{notes},\; \text{KNOWN},\\
\quad n=1,\; \text{cutoff}=0.25)[0] & \text{if matches exist} \\
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_complaint_matching.pdf}
    }}
...
The complete \texttt{match\_complaint()} function is implemented at \texttt{ml\_predictor.py:246--274}:
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_xgboost_architecture.pdf}
    }}
...
The XGBoost hyperparameters are defined at \texttt{ml\_predictor.py:409--413}:
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_cascade_oof.pdf}
    }}
...
4  & \texttt{physical\_damage}  & keyword $\in$ \{crack, broken, impact, collision, bent, misuse, dropped, phys.\ damage\} & CF  & Rejected & 88.5 \\
5  & \texttt{ntf}               & keyword $\in$ \{no fault, ntf, no trouble, no issue, no defect, intermittent, cannot reproduce\} & ATS & Approved & 95.0 \\
6  & \texttt{u\_code}           & DTC matches \texttt{\textbackslash bU[0-9A-Fa-f]\{4\}\textbackslash b} & PF  & Approved & 57.0 \\
7  & \texttt{p\_code\_engine}   & DTC matches \texttt{\textbackslash bP0[0-9]\{3\}\textbackslash b} \textbf{and} keyword $\in$ \{jerk, pickup, accel., overheat, fuel, idle, rough\} & PF  & Approved & 80.5 \\
8  & \texttt{c\_code}           & DTC matches \texttt{\textbackslash bC[0-9A-Fa-f]\{4\}\textbackslash b} & PF  & Approved & 80.0 \\
9  & \texttt{b\_code}           & DTC matches \texttt{\textbackslash bB[0-9A-Fa-f]\{4\}\textbackslash b} & PF  & Approved & 80.0 \\
\bottomrule
\end{tabular}
\end{table}
...
The complete \texttt{run\_rules()} function is implemented at \texttt{ml\_predictor.py:465--492}:
...
OpenAI is preferred when both keys are available because
\texttt{gpt-4o-mini} provides more consistent JSON output formatting.
The OpenRouter provider serves as a free-tier fallback.

Retry logic with exponential backoff handles transient API
failures, implemented in
\texttt{understand\_claim\_with\_retry()}
(\texttt{llm\_client.py:380--405}):
...
\toprule
\textbf{Service} & \textbf{Function} & \textbf{Prompt Var.} & \textbf{Output Keys} \\
\midrule
Claim understand\-ing & \texttt{understand\_claim()} (l.\,346) & \texttt{UNDERSTAND\_CLAIM\_PROMPT} (l.\,306--343) & category, normalised\_complaint, severity, failure\_analysis, reasoning, confidence \\[3pt]
Feature translation & \texttt{translate\_to\_ml\_features()} (l.\,437) & \texttt{TRANSLATE\_ML\_FEATURES\_PROMPT} (l.\,408--434) & customer\_complaint, dtc\_codes, dtc\_text, dtc\_count, has\_P/U/C/B \\[3pt]
Output formatting & \texttt{format\_output()} (l.\,273) & \texttt{FORMAT\_OUTPUT\_PROMPT} (l.\,243--270) & status, failure\_analysis, warranty\_decision, confidence, reason, matched\_complaint, decision\_engine \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_llm_integration.pdf}
    }}
...
\text{``Customer Failure''}            & \text{if cat.} \in \{\text{moisture\_damage, physical\_damage}\} \\
\text{``According to Specification''} & \text{if category} = \text{ntf} \\
\text{``Production Failure''}         & \text{if cat.} \in \{\text{electrical\_issue,}\\
                                       & \quad \text{engine\_symptom, communication\_fault}\} \\
...
The disambiguation rules, embedded in the
\texttt{UNDERSTAND\_CLAIM\_PROMPT}
(\texttt{llm\_client.py:306--343}), are applied in first-match-wins
order:

\begin{enumerate}
    \item \textbf{Engine symptom:} overheating, jerking, pickup, acceleration, fuel, idle, rough $\to$ \texttt{engine\_symptom}
    \item \textbf{Communication fault:} CAN bus, LIN bus,
          communication, network, U-code
          $\to$ \texttt{communication\_fault}
    \item \textbf{Moisture damage:} moisture, water, wet, flood,
          rain, humidity, corrosion
          $\to$ \texttt{moisture\_damage}
...
The \texttt{understand\_claim()} function
(\texttt{llm\_client.py:346--377}) sends the prompt with the claim's
technician notes and DTC code, then parses the JSON response into a
structured dictionary. The function uses default values for any missing
keys: \texttt{category="other"},
\texttt{normalized\_complaint="OBD Light ON"},
\texttt{severity="moderate"}, \texttt{failure\_analysis="Unknown"},
\texttt{reasoning=""}, \texttt{confidence=0.0} (lines 368--374).
The retry wrapper \texttt{understand\_claim\_with\_retry()}
(lines 380--405) adds the exponential backoff described in
Section~\ref{sec:llm-integration}.

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_llm_categorisation.pdf}
    }}
...
The provider selection logic is implemented at \texttt{llm\_client.py:28--33}:
...
The LLM dispatch function is implemented at \texttt{llm\_client.py:63--77}:
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_score_combination.pdf}
    }}
...
The core agreement logic from \texttt{combine\_scores()} is implemented at \texttt{ml\_predictor.py:704--737}:
...
\begin{equation}
\begin{split}
y_1 = \text{understand\_claim}(x_1) = \{&\text{category},\; \text{normalized\_complaint},\\
&\text{severity},\; \text{failure\_analysis},\; \text{reasoning},\; \text{confidence}\}
\end{split}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_stage1_llm_understanding.pdf}
    }}
...
\{\text{rule\_id},\; \text{status},\; \text{warranty\_decision},\\
\quad \text{rule\_confidence},\; \text{failure\_analysis},\; \text{reason},\; \text{rule\_fired=True}\} & \text{if match} \\
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_stage2_rule_engine.pdf}
    }}
...
The third stage translates raw claim inputs into structured ML features.
When the LLM is available, it uses semantic understanding to extract
features; otherwise, it falls back to deterministic DTC parsing and
complaint matching. This dual-path design ensures that the ML classifier
always receives a valid feature vector regardless of LLM availability.

\textbf{LLM path} (lines 868--876): The LLM translates the
claim into structured features via
\texttt{translate\_to\_ml\_features()}
(\texttt{llm\_client.py:437}), which returns a dictionary with
customer complaint, DTC codes, DTC text, DTC count, and prefix flags.
After the LLM returns, DTC one-hot features are merged from the
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_stage3_feature_translation.pdf}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_stage4_xgboost_cascade.pdf}
    }}
...
\begin{equation}
\begin{split}
y_5 = \{&\text{status},\; \text{warranty\_decision},\; \text{combined\_confidence},\\
&\text{agreement},\; \text{rule\_fired},\; \text{rule\_id},\; \text{decision\_engine}\}
\end{split}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_stage5_score_combination.pdf}
    }}
...
\textbf{LLM path} (lines 912--915): The \texttt{format\_output()}
function (\texttt{llm\_client.py:273}) sends the combined decision as
JSON to the LLM, which generates professional, technician-facing
output. The prompt (\texttt{FORMAT\_OUTPUT\_PROMPT}, lines 243--270)
constrains the LLM to produce a JSON response with exact field values
for status, warranty\_decision, confidence, matched\_complaint, and
decision\_engine, while synthesising the failure\_analysis and reason
fields from both the LLM and ML outputs.

\textbf{Fallback path} (lines 919--920):
\texttt{assemble\_output\_from\_fields()}
(\texttt{ml\_predictor.py:811--833}) constructs the output from
structured fields using template strings:

\begin{equation}
\text{reason} = \begin{cases}
\text{``Rule ''} + \text{rule\_id} + \text{`` fired. ML ''}\\
\quad + (\text{``agrees''}/\text{``disagrees''}) + \text{`` confidence ''} + \text{conf} + \text{``\%.''} & \text{if rule fired} \\[4pt]
\text{``No rule matched. ML predicts ''}\\
\quad + \text{ml\_wd} + \text{`` confidence ''} + \text{conf} + \text{``\%.''} & \text{otherwise}
\end{cases}
...
\begin{equation}
\begin{split}
\text{output} = \{&\text{status},\; \text{failure\_analysis},\; \text{warranty\_decision},\\
&\text{confidence},\; \text{reason},\; \text{matched\_complaint},\; \text{decision\_engine}\}
\end{split}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_stage6_output_formatting.pdf}
    }}
...
The \texttt{predict()} function orchestrates all six stages into a
complete prediction pipeline, handling LLM availability checks, error
recovery, and structured logging at each stage. The function spans
\texttt{ml\_predictor.py:836--925} and is the sole entry point for
...
f_6^{\text{LLM}}(f_5(f_4(f_3^{\text{LLM}}(f_2(f_1^{\text{LLM}}(x)))))) & \text{full LLM} \\
f_6^{\text{fb}}(f_5(f_4(f_3^{\text{fb}}(f_2(\text{None}))))) & \text{no LLM}
\end{cases}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_complete_predict_flow.pdf}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_classifier_evaluation.pdf}
    }}
...
The FA CV is implemented at \texttt{evaluate\_model.py:435--437} using
\texttt{cross\_val\_score(clf\_fa, X\_te, yfa\_te,}
\texttt{cv=3, scoring="accuracy", n\_jobs=-1)}.
The WD CV at lines 449--451 uses the FA-augmented feature matrix
\texttt{X\_wd\_te}.
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_cv_on_test_set.pdf}
    }}
...
The calibration check is implemented in
\texttt{check\_cascade\_calibration()}
(\texttt{evaluate\_model.py:162--190}).\linebreak
The function computes \texttt{top\_conf\_tr} and \texttt{top\_conf\_te}
at lines 171--173, the mean gap at line~183, and prints a warning at
lines 185--188 recommending the use of \texttt{cross\_val\_predict}
for OOF probabilities if the gap exceeds 0.05.

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_cascade_calibration.pdf}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_pipeline_evaluation.pdf}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fih_methodology_feature_importance.pdf}
    }}
...
\begin{equation}
\begin{split}
\text{ClaimResponse} = \{&\text{status},\; \text{failure\_analysis},\; \text{warranty\_decision},\\
&\text{confidence},\; \text{reason},\; \text{matched\_complaint},\; \text{decision\_engine}\}
\end{split}
...
\begin{tabular}{llR{3.5cm}R{3.5cm}}
\toprule
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_fastapi_architecture.pdf}
    }}
...
The Pydantic request/response schemas are defined at \texttt{main.py:89--101}:
...
The \texttt{/analyze} endpoint is defined at \texttt{main.py:112--136}:
...
\begin{multline}
\text{fetch}(\text{``http://localhost:8000/analyze''},\\
\{\text{method: ``POST''},\; \text{headers: \{``Content-Type'': ``application/json''\}},\; \text{body: JSON.stringify(request)}\})
...
\begin{equation}
\begin{split}
\text{response.json()} \to \{&\text{status},\; \text{failure\_analysis},\; \text{warranty\_decision},\\
&\text{confidence},\; \text{reason},\; \text{matched\_complaint},\; \text{decision\_engine}\}
\end{split}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_frontend_architecture.pdf}
    }}
    \caption{Frontend architecture showing the single-page application with form inputs and result display.}
    \label{fig:frontend-architecture}
\end{figure}

The JavaScript \texttt{analyzeClaim()} function is implemented at \texttt{frontend/index.html:535--571}:
...
async function analyzeClaim() {
    ...
    try {
      const resp = await fetch(API_URL, {
        ...
        body:    JSON.stringify({ fault_code, technician_notes, voltage }),
      });
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fig_docker_compose_architecture.pdf}
    }}
...
\begin{multline}
\text{format} = \text{``\%(asctime)s [\%(levelname)s] \%(name)s }\\
\text{\%(filename)s:\%(funcName)s:\%(lineno)d - \%(message)s''}
\label{eq:log-format}
\end{multline}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fih_methodology_logging_architecture.pdf}
    }}
...
The \texttt{DecisionLogger} class is implemented at \texttt{logging\_config.py:38--60}:
...
The model path is defined at line~93:
\texttt{MODEL\_PATH = os.path.join(BASE\_DIR,}
\texttt{"trace\_models.pkl")}.
The bundle is loaded lazily via the global \texttt{\_bundle} variable
(line 461), initialised on first \texttt{predict()} call
(lines 837--839).

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering
    \includegraphics[width=0.85\textwidth]{figures/fih_methodology_model_serialization.pdf}
    }}
