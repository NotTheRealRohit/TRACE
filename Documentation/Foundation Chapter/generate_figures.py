...
def plot_decision_tree_example():
    ...
    add_leaf(ax, 0.5, 4.0, "Production\nFailure", COLORS['success'])
    add_leaf(ax, 2.0, 4.0, "Customer\nFailure", COLORS['secondary'])
    add_leaf(ax, 4.5, 4.0, "Rejected", COLORS['danger'])
    add_leaf(ax, 6.0, 4.0, "Needs\nReview", COLORS['gray_medium'])
    ...
    ax.annotate('', xy=(5.0, 6.7), xytext=(4.2, 8.0), arrowprops=dict(arrowstyle='->', color=COLORS['gray_dark'], lw=1.2))
    ax.text(5.3, 7.5, "N", fontsize=7, color=COLORS['danger'])
    
    ax.annotate('', xy=(1.35, 4.7), xytext=(1.9, 6.0), arrowprops=dict(arrowstyle='->', color=COLORS['gray_dark'], lw=1.2))
    ax.text(1.1, 5.5, "Y", fontsize=7, color=COLORS['success'])
    ax.annotate('', xy=(2.15, 4.7), xytext=(2.0, 6.0), arrowprops=dict(arrowstyle='->', color=COLORS['gray_dark'], lw=1.2))
    ax.text(2.4, 5.5, "N", fontsize=7, color=COLORS['danger'])
    
    ax.annotate('', xy=(5.35, 4.7), xytext=(5.0, 6.0), arrowprops=dict(arrowstyle='->', color=COLORS['gray_dark'], lw=1.2))
    ax.text(5.1, 5.5, "Y", fontsize=7, color=COLORS['success'])
    ax.annotate('', xy=(6.15, 4.7), xytext=(6.0, 6.0), arrowprops=dict(arrowstyle='->', color=COLORS['gray_dark'], lw=1.2))
    ax.text(6.4, 5.5, "N", fontsize=7, color=COLORS['danger'])
...
def plot_gradient_boosting_sequential():
    ...
    labels = ["F0(x) = mean(y)", "Tree h1 to r1", "Tree h2 to r2"]
    
    for i, (y, label) in enumerate(zip(y_positions, labels)):
        add_box(ax, 2.0, y - 0.35, 2.2, 0.7, label, facecolor=COLORS['bg_box'], edgecolor=COLORS['primary'], fontsize=7)
        
        if i == 0:
            ax.annotate('', xy=(3.35, 6.5), xytext=(3.35, 7.15), arrowprops=dict(arrowstyle='->', color=COLORS['danger'], lw=1.2))
            ax.text(3.8, 6.8, "r1 = y - F0(x)", ha='center', fontsize=6, color=COLORS['danger'])
        else:
            ax.annotate('', xy=(3.35, y - 0.35), xytext=(3.35, y - 1.0), arrowprops=dict(arrowstyle='->', color=COLORS['danger'], lw=1.2))
            if i == 1:
                ax.text(3.8, y - 0.6, "r1", ha='center', fontsize=6, color=COLORS['danger'])
    
    add_box(ax, 4.5, 7.5 - 0.35, 1.8, 0.7, "F1 = F0 + eta*h1", facecolor=COLORS['purple'], edgecolor=COLORS['purple'], fontsize=7, fontweight='bold')
    add_box(ax, 4.5, 5.5 - 0.35, 1.8, 0.7, "F2 = F1 + eta*h2", facecolor=COLORS['purple'], edgecolor=COLORS['purple'], fontsize=7, fontweight='bold')
    
    ax.annotate('', xy=(4.2, 7.15), xytext=(4.2, 7.5), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1))
    ax.annotate('', xy=(4.5, 5.15), xytext=(4.2, 5.15), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1))
    ax.annotate('', xy=(6.0, 7.15), xytext=(6.0, 6.0), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1, ls='--'))
    
    ax.text(5.0, 2.0, "Learning rate eta controls contribution", ha='center', fontsize=7, style='italic')
...
def plot_cascade_classifier():
    ...
    add_box(ax, 0.5, 5.0, 1.8, 0.9, "Input x", facecolor=COLORS['bg_box'], edgecolor=COLORS['primary'], fontsize=9)
    
    ax.annotate('', xy=(2.65, 5.45), xytext=(2.45, 5.45), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 2.8, 5.0, 2.2, 0.9, "Classifier f1\n(Failure Analysis)", facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(5.5, 5.45), xytext=(5.0, 5.45), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    ax.text(4.75, 5.7, "softmax", ha='center', fontsize=7)
    
    add_box(ax, 5.5, 5.0, 0.9, 0.9, "p", facecolor=COLORS['bg_box'], edgecolor=COLORS['purple'], fontsize=9)
    
    rect = FancyBboxPatch((2.0, 3.5), 5.0, 1.3, boxstyle="round,pad=0.08", facecolor='none', edgecolor=COLORS['purple'], linewidth=1, linestyle='--')
    ax.add_patch(rect)
    ax.text(4.5, 4.0, "Augmented: x' = [x | p]", ha='center', fontsize=7, color=COLORS['purple'])
    
    ax.annotate('', xy=(4.5, 5.0), xytext=(4.5, 4.4), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 2.8, 2.0, 2.2, 0.9, "Classifier f2\n(Warranty Decision)", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(5.5, 2.45), xytext=(5.0, 2.45), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 5.5, 2.0, 1.8, 0.9, "Final\nPrediction", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=8, fontweight='bold')
    
    ax.text(1.4, 7.5, "Stage 1", ha='center', fontsize=8, color=COLORS['primary'], fontweight='bold')
    ax.text(4.5, 1.2, "Stage 2", ha='center', fontsize=8, color=COLORS['success'], fontweight='bold')
...
def plot_tfidf_computation():
    ...
    boxes = [
        (0.5, 7.0, "Raw Text", "engine\noverheating"),
        (2.2, 7.0, "Tokenize", "['engine'"),
        (4.0, 7.0, "TF Comp", "count/total"),
        (5.8, 7.0, "IDF", "log(N/df)"),
        (7.6, 7.0, "TF-IDF", "TF x IDF"),
    ]
    
    for x, y, title, content in boxes:
        add_box(ax, x, y, 1.2, 1.0, title, facecolor=COLORS['bg_box'], edgecolor=COLORS['primary'], fontsize=7)
        ax.text(x + 0.6, y - 0.25, content, ha='center', fontsize=5, family='monospace')
    
    for i in range(len(boxes)-1):
        ax.annotate('', xy=(boxes[i+1][0], 7.9), xytext=(boxes[i][0]+1.2, 7.9), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    ax.text(3.25, 5.5, "TF(t,d) = count/total", ha='center', fontsize=7, style='italic')
    ax.text(3.25, 5.0, "IDF(t) = log(N/df) + 1", ha='center', fontsize=7, style='italic')
    ax.text(3.25, 4.5, "TF-IDF = TF x IDF", ha='center', fontsize=7, style='italic')
...
def plot_feature_binning():
    ...
    ax.text(2.5, max(ax.get_ylim()) * 0.3, "K=5 equal-width bins", ha='center', fontsize=8, style='italic')
...
def plot_rule_engine_flow():
    ...
    diamond_y = 8.0
    add_diamond(ax, 2.5, diamond_y, 0.7, "Input")
    ...
    y = diamond_y - 1.2
    for rule_text, y_pos in rules:
        add_diamond(ax, 2.5, y_pos, 0.5, rule_text)
        ax.annotate('', xy=(2.5, y_pos - 0.5), xytext=(2.5, y - 0.2), arrowprops=dict(arrowstyle='->', color=COLORS['gray_dark'], lw=1.2))
        
        add_box(ax, 0.5, y_pos - 0.4, 0.9, 0.5, "Yes", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=7, fontweight='bold')
        ax.annotate('', xy=(1.55, y_pos - 0.25), xytext=(1.35, y_pos - 0.25), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
        
        ax.text(3.6, y_pos - 0.15, "N", fontsize=6, color=COLORS['danger'])
        
        y = y_pos - 0.9
    
    add_box(ax, 3.2, 0.8, 1.3, 0.5, "Default/ML", facecolor=COLORS['gray_medium'], edgecolor=COLORS['gray_medium'], fontsize=7, fontweight='bold')
    ax.annotate('', xy=(3.85, 1.3), xytext=(2.5, 1.3), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
...
def plot_transformer_architecture():
    ...
    fig, ax = create_figure(5.0, 5.0)
    
    add_box(ax, 1.0, 0.8, 3.0, 0.6, "Input + Positional Encoding", facecolor=COLORS['bg_box'], edgecolor=COLORS['primary'], fontsize=7)
    
    for i in range(2):
        y = 2.3 + i * 1.6
        
        add_box(ax, 0.8, y, 3.4, 0.5, "Multi-Head Attention", facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=7, fontweight='bold')
        
        ax.annotate('', xy=(2.5, y + 0.5), xytext=(2.5, y + 0.1), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
        
        add_box(ax, 0.8, y - 0.7, 3.4, 0.5, "Add & Norm", facecolor=COLORS['purple'], edgecolor=COLORS['purple'], fontsize=7, fontweight='bold')
        
        ax.annotate('', xy=(2.5, y - 0.2), xytext=(2.5, y - 0.7), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
        
        add_box(ax, 0.8, y - 1.4, 3.4, 0.5, "Feed Forward", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=7, fontweight='bold')
        
        add_box(ax, 0.8, y - 2.1, 3.4, 0.5, "Add & Norm", facecolor=COLORS['purple'], edgecolor=COLORS['purple'], fontsize=7, fontweight='bold')
        
        if i == 0:
            ax.annotate('', xy=(2.5, y - 1.4), xytext=(2.5, y - 2.1), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
    
    ax.text(4.0, 3.8, "N x", ha='center', fontsize=9, fontweight='bold', color=COLORS['gray_medium'])
    
    add_box(ax, 1.0, 8.0, 3.0, 0.6, "Output Representations", facecolor=COLORS['bg_box'], edgecolor=COLORS['success'], fontsize=7)
    ax.annotate('', xy=(2.5, 7.75), xytext=(2.5, 7.25), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    ax.text(4.3, 4.1, "Residual", ha='left', fontsize=6, style='italic', color=COLORS['gray_medium'])
...
def plot_prompt_structure():
    ...
    sections = [
        (8.0, "Role", "You are expert...", COLORS['primary']),
        (6.5, "Input", "fault_code...", COLORS['success']),
        (5.0, "Constraints", "category: ...", COLORS['secondary']),
        (3.5, "Rules", "overheat -> ...", COLORS['purple']),
        (2.0, "Format", "{JSON}", COLORS['brown']),
    ]
    
    for y, title, content, color in sections:
        add_box(ax, 0.5, y - 0.6, 4.5, 1.0, title, facecolor=color, edgecolor=color, fontsize=8, fontweight='bold')
        ax.text(2.75, y - 0.2, content, ha='center', fontsize=5, family='monospace')
    
    ax.text(5.5, 8.5, "Prompt Components (top to bottom)", ha='center', fontsize=9, fontweight='bold')
...
def plot_api_integration_flow():
    ...
    add_box(ax, 0.5, 3.5, 1.6, 0.9, "Client App", facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(2.5, 3.95), xytext=(2.1, 3.95), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 2.7, 3.5, 2.2, 0.9, "HTTP POST\n/chat/completions", facecolor=COLORS['gray_medium'], edgecolor=COLORS['gray_medium'], fontsize=7, fontweight='bold')
    
    ax.annotate('', xy=(5.05, 3.95), xytext=(4.85, 3.95), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 5.2, 3.5, 1.6, 0.9, "API Server\n(LLM)", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(4.5, 3.0), xytext=(4.5, 2.6), arrowprops=dict(arrowstyle='->', color=COLORS['purple'], lw=1.2, ls='--'))
    
    add_box(ax, 3.2, 1.3, 2.2, 0.9, "JSON Response", facecolor=COLORS['purple'], edgecolor=COLORS['purple'], fontsize=8, fontweight='bold')
    
    ax.text(1.3, 4.7, "POST /v1/chat/completions", ha='center', fontsize=6, color=COLORS['secondary'])
    ax.text(1.3, 2.9, "Authorization: Bearer", ha='center', fontsize=5, color=COLORS['gray_medium'], style='italic')
...
def plot_precision_recall_f1():
    ...
    ax.axhline(0.85, color=COLORS['purple'], linestyle='--', linewidth=1.5)
    
    ax.set_ylabel('Score', fontsize=9)
    ax.set_ylim(0, 1.1)
    style_axes(ax)
    
    ax.text(4.0, 0.5, 'F1 = 2*P*R/(P+R)', ha='center', fontsize=8, style='italic')
...
def plot_feature_importance():
    ...
    ax2.plot(cumulative, y_pos, color=COLORS['danger'], linewidth=2, marker='o', markersize=3)
...
def plot_pipeline_architecture():
    ...
    stages = [
        (0.5, "Input", "Raw"),
        (1.9, "Preprocess", "Clean"),
        (3.3, "Features", "Vector"),
        (4.7, "Inference", "Preds"),
        (6.1, "Post-proc", "Output"),
        (7.5, "Output", "JSON"),
    ]
    
    for i, (x, name, data) in enumerate(stages):
        add_box(ax, x, 4.2, 1.1, 0.7, name, facecolor=COLORS['bg_box'], edgecolor=COLORS['primary'], fontsize=7)
        ax.text(x + 0.55, 3.9, data, ha='center', fontsize=5, family='monospace')
        
        if i < len(stages) - 1:
            ax.annotate('', xy=(stages[i+1][0], 4.55), xytext=(x + 1.1, 4.55), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
            ax.text((x + stages[i+1][0])/2 + 0.55, 5.0, str(i+1), ha='center', fontsize=6, color=COLORS['gray_medium'])
    
    ax.text(4.0, 8.0, "Pipeline Stage", ha='center', fontsize=8, fontweight='bold')
    ax.text(4.0, 7.5, "Data Format", ha='center', fontsize=7, style='italic')
...
def plot_hybrid_engine():
    ...
    sources = [
        (0.8, 6.0, "Rule Engine", "f_r(x)", COLORS['primary']),
        (0.8, 4.5, "ML Classifier", "f_ml(x)", COLORS['success']),
        (0.8, 3.0, "LLM Layer", "f_llm(x)", COLORS['purple']),
    ]
    
    for x, y, label, eq, color in sources:
        add_box(ax, x, y, 1.8, 0.7, label, facecolor=color, edgecolor=color, fontsize=8, fontweight='bold')
        ax.text(x + 0.9, y - 0.35, eq, ha='center', fontsize=6, family='monospace')
        
        ax.annotate('', xy=(4.0, y + 0.35), xytext=(2.6, y + 0.35), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 4.5, 4.3, 1.8, 1.0, "Weighted\nCombiner", facecolor=COLORS['secondary'], edgecolor=COLORS['secondary'], fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(6.3, 4.8), xytext=(5.4, 4.8), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 6.8, 4.3, 1.6, 1.0, "Final\nDecision", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=8, fontweight='bold')
    ax.text(7.6, 4.8, "Decision", ha='left', fontsize=6)
    
    ax.text(3.2, 7.0, "Agreement Logic", ha='center', fontsize=7, style='italic', color=COLORS['gray_medium'])
...
def plot_model_serialization():
    ...
    add_box(ax, 0.5, 4.5, 2.2, 1.0, "Python Objects\n(theta, phi)", facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(3.2, 5.0), xytext=(2.7, 5.0), arrowprops=dict(arrowstyle='->', color=COLORS['success'], lw=1.5))
    ax.text(2.95, 5.4, "pickle.dump()", ha='center', fontsize=7, color=COLORS['success'])
    
    add_box(ax, 3.7, 4.5, 1.3, 1.0, ".pkl\nFile", facecolor=COLORS['white'], edgecolor=COLORS['gray_dark'], fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(5.25, 5.0), xytext=(5.05, 5.0), arrowprops=dict(arrowstyle='->', color=COLORS['danger'], lw=1.2))
    ax.text(5.0, 3.7, "pickle.load()", ha='center', fontsize=7, color=COLORS['danger'])
    
    add_box(ax, 5.5, 4.5, 2.2, 1.0, "Restored Objects\n(theta, phi)", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=8, fontweight='bold')
    
    ax.text(4.3, 3.5, "trace_models.pkl", ha='center', fontsize=8, fontweight='bold', color=COLORS['gray_dark'])
    ax.text(4.3, 3.0, "Serialize / Deserialize", ha='center', fontsize=7, style='italic')
...
def plot_rest_api():
    ...
    add_box(ax, 0.5, 5.0, 1.6, 0.9, "Client\n(Browser)", facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(2.75, 5.45), xytext=(2.55, 5.45), arrowprops=dict(arrowstyle='->', color=COLORS['secondary'], lw=1.5))
    ax.text(1.8, 5.8, "POST /analyze", ha='center', fontsize=7, fontweight='bold', color=COLORS['secondary'])
    
    add_box(ax, 2.9, 5.0, 2.2, 0.9, "API Server\n(FastAPI)", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=8, fontweight='bold')
    
    add_box(ax, 2.9, 3.3, 2.2, 0.9, "JSON Response", facecolor=COLORS['purple'], edgecolor=COLORS['purple'], fontsize=8, fontweight='bold')
    ax.annotate('', xy=(3.4, 4.1), xytext=(3.4, 4.5), arrowprops=dict(arrowstyle='->', color=COLORS['purple'], lw=1.2))
    
    ax.text(0.8, 7.0, "Request", ha='center', fontsize=7, color=COLORS['secondary'])
    ax.text(0.8, 3.7, "Response", ha='center', fontsize=7, color=COLORS['purple'])
    
    ax.text(4.5, 7.0, "Content-Type: app/json", ha='center', fontsize=5, style='italic', color=COLORS['gray_medium'])
...
def plot_file_upload_flow():
    ...
    steps = [
        (0.5, "Client Browser", COLORS['primary']),
        (2.2, "multipart/form", COLORS['secondary']),
        (3.9, "Server Parser", COLORS['success']),
        (5.6, "Processing", COLORS['purple']),
    ]
    
    for i, (x, label, color) in enumerate(steps):
        add_box(ax, x, 5.0, 1.4, 0.9, label, facecolor=color, edgecolor=color, fontsize=7, fontweight='bold')
        
        if i < len(steps) - 1:
            ax.annotate('', xy=(steps[i+1][0] + 0.05, 5.45), xytext=(x + 1.55, 5.45), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    ax.text(1.5, 6.5, "boundary=...", ha='center', fontsize=5, family='monospace', color=COLORS['gray_medium'])
    ax.text(3.0, 3.8, "file stream", ha='center', fontsize=6, style='italic')
    ax.text(4.8, 3.8, "result", ha='center', fontsize=6, style='italic')
...
def plot_ocr_pipeline():
    ...
    rect = FancyBboxPatch((0.5, 5.0), 1.8, 1.3, boxstyle="round,pad=0.05", facecolor=COLORS['gray_medium'], edgecolor=COLORS['gray_dark'], linewidth=1)
    ax.add_patch(rect)
    ax.text(1.4, 6.1, "Input Image", ha='center', fontsize=7)
    ax.text(1.4, 5.4, "P0562 14.2V", ha='center', fontsize=8, fontweight='bold', family='monospace')
    
    ax.annotate('', xy=(2.65, 5.65), xytext=(2.45, 5.65), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 3.1, 5.15, 1.3, 0.9, "Text Detection", facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=7, fontweight='bold')
    
    for xi, yi in [(0.7, 5.8), (1.3, 6.2), (1.9, 5.5)]:
        rect = FancyBboxPatch((xi - 0.12, yi - 0.12), 0.4, 0.25, boxstyle="square", facecolor='none', edgecolor=COLORS['primary'], linewidth=1.5)
        ax.add_patch(rect)
    
    ax.annotate('', xy=(4.55, 5.6), xytext=(4.35, 5.6), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 4.9, 5.15, 1.3, 0.9, "Character\nRecognition", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=7, fontweight='bold')
    
    ax.annotate('', xy=(6.35, 5.6), xytext=(6.15, 5.6), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    add_box(ax, 6.7, 5.15, 1.3, 0.9, "Output Text", facecolor=COLORS['gray_dark'], edgecolor=COLORS['gray_dark'], fontsize=7, fontweight='bold')
    
    ax.text(1.4, 4.0, "Input", ha='center', fontsize=7, color=COLORS['gray_medium'])
    ax.text(4.0, 4.0, "f_detect", ha='center', fontsize=7, color=COLORS['primary'])
    ax.text(5.8, 4.0, "f_recognize", ha='center', fontsize=7, color=COLORS['success'])
    ax.text(7.4, 4.0, "Output", ha='center', fontsize=7, color=COLORS['gray_dark'])
...
def plot_logging_architecture():
    ...
    add_box(ax, 1.6, 7.5, 1.8, 0.7, "root Logger", facecolor=COLORS['gray_dark'], edgecolor=COLORS['gray_dark'], fontsize=8, fontweight='bold')
    
    add_box(ax, 1.6, 6.0, 1.8, 0.7, "trace", facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=8, fontweight='bold')
    ax.annotate('', xy=(2.5, 7.5), xytext=(2.5, 6.7), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    ...
    for x, y, name in loggers:
        add_box(ax, x, y, 1.8, 0.7, name, facecolor=COLORS['primary'], edgecolor=COLORS['primary'], fontsize=7)
        ax.annotate('', xy=(2.5, 6.0), xytext=(x + 0.9, 5.2), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
    ...
    for x, y, name in handlers:
        add_box(ax, x, y, 1.6, 0.6, name, facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=7, fontweight='bold')
        ax.annotate('', xy=(2.5, 4.0), xytext=(2.5, 4.5), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
    
    add_box(ax, 4.2, 2.8, 1.2, 0.6, "Formatter", facecolor=COLORS['purple'], edgecolor=COLORS['purple'], fontsize=7, fontweight='bold')
    
    ax.annotate('', xy=(4.8, 3.4), xytext=(3.0, 3.4), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
...
def plot_docker_layers():
    ...
    layers = [
        (8.0, "ubuntu:22.04", "Base OS", COLORS['primary']),
        (6.8, "Python 3.11", "Runtime", COLORS['success']),
        (5.6, "pip install", "Deps", COLORS['secondary']),
        (4.4, "COPY . /app", "App Code", COLORS['purple']),
        (3.2, "CMD uvicorn", "Entry", COLORS['brown']),
    ]
    
    for y, cmd, label, color in layers:
        rect = FancyBboxPatch((0.8, y - 0.4), 2.8, 0.8, boxstyle="round,pad=0.05", facecolor=color, edgecolor=color, alpha=0.3, linewidth=1.2)
        ax.add_patch(rect)
        ax.text(2.2, y, cmd, ha='center', va='center', fontsize=7, fontweight='bold')
        ax.text(4.8, y, label, ha='left', va='center', fontsize=8, fontweight='bold')
    
    ax.annotate('', xy=(4.2, 2.7), xytext=(4.2, 2.0), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
    ax.text(4.2, 1.7, "Image", ha='center', fontsize=8, fontweight='bold')
    
    rect = FancyBboxPatch((0.6, 1.3), 4.2, 7.8, boxstyle="round,pad=0.05", facecolor='none', edgecolor=COLORS['border'], linewidth=2)
    ax.add_patch(rect)
    
    rect = FancyBboxPatch((0.6, 1.3), 4.2, 0.45, boxstyle="round,pad=0.05", facecolor=COLORS['success'], edgecolor=COLORS['success'], alpha=0.3, linewidth=1)
    ax.add_patch(rect)
    ax.text(2.4, 1.5, "Container (writable)", ha='center', va='center', fontsize=7, fontweight='bold')
...
def plot_health_check_cycle():
    ...
    fig, ax = create_figure(4.0, 4.0)
    
    steps = [
        (2.5, 7.5, "Send Probe", COLORS['primary']),
        (5.0, 5.5, "Evaluate\nResponse", COLORS['success']),
        (5.0, 3.2, "Healthy?", COLORS['secondary']),
    ]
    
    for x, y, label, color in steps:
        add_box(ax, x - 0.7, y - 0.35, 1.4, 0.7, label, facecolor=color, edgecolor=color, fontsize=7, fontweight='bold')
    
    ax.annotate('', xy=(5.0, 6.6), xytext=(2.5, 7.1), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    ax.annotate('', xy=(5.0, 4.3), xytext=(5.0, 4.8), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=1.2))
    
    ax.annotate('', xy=(3.5, 3.2), xytext=(5.0, 3.2), arrowprops=dict(arrowstyle='->', color=COLORS['success'], lw=1.2))
    ax.text(4.0, 3.5, "Yes", ha='center', fontsize=6, color=COLORS['success'], fontweight='bold')
    
    add_box(ax, 1.0, 2.7, 1.2, 0.55, "Wait\ninterval", facecolor=COLORS['success'], edgecolor=COLORS['success'], fontsize=6)
    ax.annotate('', xy=(2.2, 3.0), xytext=(2.2, 2.7), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8))
    ax.annotate('', xy=(2.5, 3.0), xytext=(2.5, 7.1), arrowprops=dict(arrowstyle='->', color=COLORS['gray_medium'], lw=0.8, ls='--'))
    
    ax.annotate('', xy=(5.0, 2.6), xytext=(5.0, 2.0), arrowprops=dict(arrowstyle='->', color=COLORS['danger'], lw=1.2))
    ax.text(5.25, 2.4, "No", ha='left', fontsize=6, color=COLORS['danger'], fontweight='bold')
    
    add_box(ax, 4.3, 0.9, 1.4, 0.55, "Restart", facecolor=COLORS['danger'], edgecolor=COLORS['danger'], fontsize=6, fontweight='bold')
    
    ax.text(2.5, 0.8, "interval=30s", ha='center', fontsize=7, style='italic')
